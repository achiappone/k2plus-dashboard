...
<form id="f"><h1>Chauvet's Print Lab</h1>
...
<title>Chauvet's Print Lab</title>
...
@media(max-width:900px){
  ...
  #progresscard .hero .big{font-size:17px;min-width:4.2ch}
...
.hero .big{font-family:"IBM Plex Mono",monospace;font-size:35px;font-weight:500;
...
/* .v is a <p>, so without this it carries the UA default 1em top AND bottom -
   ...
   twice as tall as the text in them. */
.tile .v{font-size:13px;margin:2px 0 0}
...
/* Thermals and the per-sensor sparks share a row. Both were full-width bands
   holding the same six sensors, one as numbers and one as shapes, so reading
   them meant scrolling between two views of one thing. align-items:start so the
   short table does not stretch to the taller sparks card. */
...
table.th td{padding:4px 10px;border-bottom:1px solid var(--rule-2);vertical-align:middle}
...
.morebtn{font-family:"IBM Plex Sans Condensed",sans-serif;font-size:12px;
  letter-spacing:.06em;text-transform:uppercase;background:var(--surface-1);
...
<div class="wrap">
<header>
  <h1>Chauvet&#39;s Print Lab <span class="h1sub">(Creality K2 Plus)</span></h1><span class="host" id="host">—</span>
...
function drawChart(){
  // The combined chart was removed - every sensor now has its own scale in the
  // card below, which is the only one that was readable anyway. Left as a
  // no-op rather than deleted so the resize handler, the poll loop and the
  // error path do not each need a null check.
  const box = el("chart");
  if(!box) return;
  ...
  svg.addEventListener("pointermove", e=>{
    ...
    const t=new Date(now-(n-1-i)*1000);
    tip.innerHTML = `<b>${h12(t, true)}</b><br>`
      + series.map(s=>{
...
/* 12-hour wall clock. Every time-of-day readout on this page goes through
   here so the chart axis, the tooltip, the ETA and the job list cannot drift
   into different formats. Durations stay as they were - "2h 05m" is not a
   time of day and gains nothing from am/pm. */
function h12(d, withSecs){
  let h = d.getHours();
  const ap = h < 12 ? "am" : "pm";
  h = h % 12 || 12;
  const p = n => String(n).padStart(2,"0");
  return h + ":" + p(d.getMinutes()) + (withSecs ? ":" + p(d.getSeconds()) : "") + ap;
}
function drawClock(){
  if(!clock){ return; }
...
/* Both lists can run to dozens of rows, which pushed everything below them off
   the screen. Show the newest few, keep the rest one click away. The button is
   rebuilt with the list on every poll, so it cannot outlive its rows. */
function collapse(wrap, keep, noun){
  ...
  b.className = "morebtn";
  const label = open => open ? `hide ${hidden.length} older ${noun}`
                             : `show ${hidden.length} more ${noun}`;
  let open = false;
    ...
    for(const j of jobs){
      ...
      d.innerHTML =
        `<span class="hwhen">${String(when.getMonth()+1).padStart(2,"0")}-${
          String(when.getDate()).padStart(2,"0")} ${h12(when)}</span>` +
